...
const Appointment = () => {
    ...
    const patient = useSelector((state) => state.auth.profile);
    const doctor = useSelector((state) => state.data_red.appoint_doc);
    const [value, onChange] = useState(new Date());
    const [time, onTime] = useState("");
    var date = value.getFullYear()+'-'+(value.getMonth()+1)+'-'+value.getDate();
    const appoint = {
      "p_id": patient._id , "p_name": patient.first_name+" "+patient.last_name, "p_cin": "12345678", "p_gender": patient.gender, "p_phone_number": "50505858", 
      "d_id": (doctor)?doctor._id:"" , "d_name":(doctor)?(doctor.first_name+" "+doctor.last_name):"" ,"d_speciality": (doctor)?doctor.speciality:"", "d_address": (doctor)?doctor.address:"",
